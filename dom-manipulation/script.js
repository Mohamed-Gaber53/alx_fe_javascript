...
/* =========================
   ...
   function updateCategoryOptions() {
     const categories = ["all", ...new Set(quotes.map(q => q.category || "General"))];
   ...
   function showRandomQuote() {
     ...
     if (cat !== "all") filtered = quotes.filter(q => q.category === cat);
   ...
   async function syncQuotes() {
     try {
       ...
       const {added, conflicts} = mergeQuotesFromServer(serverQuotes);
       document.getElementById("syncStatus").textContent =
         "Last sync: " + new Date().toLocaleString();
   
       if (conflicts > 0) {
         alert(`Conflicts detected: ${conflicts}`);
       } else {
